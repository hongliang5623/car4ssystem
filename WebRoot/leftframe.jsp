...
{ 
output += "<li id=" + outlookbar.itemlist[i][j].sortname + j + " onclick=\"changeframe('"+outlookbar.itemlist[i][j].title+"', '"+outlookbar.titlelist[i].title+"', '"+outlookbar.itemlist[i][j].key+"')\"><a>" + outlookbar.itemlist[i][j].title + "</a></li>" 
} 
output += "</ul></div>" 
} 
} 
...
document.write("<script src=js/nav.js></" + "script>");
  ...
  <body onload="initinav('售后简介')"> 
	<div id="left_content">
	<%
  HttpSession sessions=request.getSession(); 
  int i=Integer.parseInt(sessions.getAttribute("value").toString());
  	if(i==1){
  	Carowner o=(Carowner)sessions.getAttribute("user");
  	String truename=o.getOwerTruename();
   %>
    	 <div id="user_info">欢迎您，<strong><%=truename%></strong><br />[<a>尊敬的车主</a>，<a>退出</a>]</div>
	<%}else{
	User u=(User)sessions.getAttribute("user");
	String truename=u.getTruename();
	%>
	
     	 <div id="user_info">欢迎您，<strong><%=truename%></strong><br />[<a>系统管理员</a>，<a>退出</a>]</div>
	 	<%}%>
